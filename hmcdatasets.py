...
def load_shades_data(random_seed=1):
    ...
    for seed in seeds:
        for i in range(0, int(100 + 100 * random.random())):
            ...
            label_row["label"] = []
            label_row["label"].append(seed["node"])
            if seed["node"] == "light":
                label_row["label"].append("colors")
            if seed["node"] == "dark":
                label_row["label"].append("colors")
            if seed["node"] == "white":
                label_row["label"].append("light")
            if seed["node"] == "black":
                label_row["label"].append("dark")
            if seed["node"] == "gray":
                label_row["label"].append("dark")
            if seed["node"] == "slate":
                label_row["label"].append("gray")
            if seed["node"] == "ash":
                label_row["label"].append("gray")
            # if seed["node"] == "darkgray":
            #     label_row["label"].append ("dark")
            # if seed["node"] == "darkgray":
            #     label_row["label"].append ("gray")
